...
export default async function handler(req, res) {
  try {
    ...
    if (q) {
      // basic search
      query = query.or(
        `name.ilike.%${q}%,whatsapp.ilike.%${q}%,business_type.ilike.%${q}%,budget_range.ilike.%${q}%,start_timeline.ilike.%${q}%,motivation.ilike.%${q}%,source.ilike.%${q}%,email.ilike.%${q}%,business.ilike.%${q}%,message.ilike.%${q}%`
      );
    }
